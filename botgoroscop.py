...
# Указываем токен
bot = telebot.TeleBot('522578178:AAEcBU3GXwjSfRsmmxSDHtdn5Sk66d4nbYs')
...
def get_text_messages(message):
    ...
    if message.text == "Привет":
        ...
        key_bliznecy = types.InlineKeyboardButton(text='Близнецы', callback_data='zodiak')
        ...
        key_vodoley = types.InlineKeyboardButton(text='Водолей', callback_data='zodiaс_vod')
...
def callback_worker(call):
    ...
    if call.data == "zodiak": 
        # Формируем гороскоп
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third) + ' ' + "ГЛАВНОЕ!!! Если Вас зовут Марина Геннадьевна, необходимо бесперекословно слушаться мужа и исполнять все его пожелания!!!"
        # Отправляем текст в Телеграм
        bot.send_message(call.message.chat.id, msg)
    if call.data == "zodiaс_vod": 
        # Формируем гороскоп
        msg = random.choice(first) + ' ' + random.choice(second) + ' ' + random.choice(second_add) + ' ' + random.choice(third) + '\n\n' + "ГЛАВНОЕ!!! Если Вас зовут Иван Михайлович, необходимо срочно перевести соточку Андрюхе на карту!!!"
        # Отправляем текст в Телеграм
        bot.send_message(call.message.chat.id, msg)
